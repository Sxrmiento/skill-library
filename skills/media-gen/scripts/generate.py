...
FAL_ALLOWED_HOSTS = {
    "fal.run", "storage.googleapis.com", "fal-cdn.com",
    "cdn.fal.ai", "fal.ai",
}
FAL_ID_RE = re.compile(r"^[\w][\w\-]*/[\w\-][\w\-./]*$")
...
def _safe_url(url: str) -> str:
    """Validate that a URL is https and from an allowed Fal host."""
    parsed = urllib.parse.urlparse(url)
    if parsed.scheme != "https":
        raise ValueError(f"Unsafe URL scheme '{parsed.scheme}': only https allowed.")
    host = parsed.netloc.split(":")[0].lower()
    if not any(host == h or host.endswith("." + h) for h in FAL_ALLOWED_HOSTS):
        raise ValueError(f"URL host '{host}' is not in the Fal allowed-host list.")
    return url


def _safe_dest(dest: Path, output_dir: Path) -> Path:
    """Resolve dest and verify it is inside output_dir."""
    resolved = dest.resolve()
    base = output_dir.resolve()
    if not str(resolved).startswith(str(base) + os.sep) and resolved != base:
        raise ValueError(f"Destination '{resolved}' is outside output dir '{base}'.")
    return resolved


def download(url: str, dest: Path, output_dir: Path) -> None:
    _safe_url(url)
    _safe_dest(dest, output_dir)
...
def cmd_image(args) -> None:
    ...
    fal_id = spec["fal_id"]
    if not FAL_ID_RE.match(fal_id):
        sys.stderr.write(f"ERROR: fal_id '{fal_id}' in models.json has an unexpected format. Edit models.json to fix it.\n")
        sys.exit(4)
    ...
    if args.input_image:
        for raw in args.input_image:
            ip = expand_path(raw).resolve()
            if not ip.is_file():
                sys.stderr.write(f"ERROR: input image not found: {ip}\n")
                sys.exit(5)
            if ip.suffix.lower() not in IMAGE_EXTS:
                sys.stderr.write(f"ERROR: '{ip.name}' is not a supported image type ({', '.join(IMAGE_EXTS)}).\n")
                sys.exit(5)
    ...
    try:
        ...
        safe = {k: result[k] for k in ("status", "error", "detail") if k in result}
        sys.stderr.write(
            f"ERROR: couldn't resolve output_path '{spec['output_path']}' on result.\n"
            f"Result fields: {json.dumps(safe)}\n"
        )
        raise

    download(url, out_path, output_dir)
...
def cmd_video(args) -> None:
    ...
    fal_id = spec["fal_id"]
    if not FAL_ID_RE.match(fal_id):
        sys.stderr.write(f"ERROR: fal_id '{fal_id}' in models.json has an unexpected format.\n")
        sys.exit(4)

    folder = expand_path(args.folder).resolve()
    if not folder.is_dir():
        sys.stderr.write(f"ERROR: folder does not exist: {folder}\n")
        sys.exit(5)

    image_path = expand_path(args.image).resolve()
    if not image_path.is_file():
        sys.stderr.write(f"ERROR: image not found: {image_path}\n")
        sys.exit(5)
    if image_path.suffix.lower() not in IMAGE_EXTS:
        sys.stderr.write(f"ERROR: '{image_path.name}' is not a supported image type.\n")
        sys.exit(5)
...
def cmd_upscale(args) -> None:
    ...
    if args.factor is not None:
        ...
        if not math.isfinite(factor) or factor <= 0:
            sys.stderr.write("ERROR: --factor must be a positive finite number.\n")
            sys.exit(6)
    ...
    try:
        ...
        # Always clean up temp fps file, even on error.
